...
export function PickupGemotestScreen() {
  const { token } = useAuth();
  ...
  const loadPoints = async () => {
    ...
    try {
      ...
      const [pointsResponse, countResponse] = await Promise.all([
        axios.get(`/api/trpc/hemotest.pickupPoints?input=${encodeURIComponent(JSON.stringify(input))}`, { withCredentials: true }),
        axios.get(`/api/trpc/hemotest.pickedCount?input=${encodeURIComponent(JSON.stringify(input))}`, { withCredentials: true }),
      ]);
  ...
  const handleTogglePickup = async (pointId: number) => {
    ...
    if (!token) return;
    try {
      await axios.post('/api/trpc/hemotest.togglePickup', { json: { token, pointId, date: selectedDate.toISOString() } }, { withCredentials: true });

...
interface PickupSberbankScreenProps {
...
export function PickupSberbankScreen({ onNavigate: _onNavigate }: PickupSberbankScreenProps) {
  void _onNavigate;
  const { token } = useAuth();
  ...
  const loadPoints = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `/api/trpc/sberbank.pickupPoints?input=${encodeURIComponent(JSON.stringify({ token, date: selectedDate }))}`,
  ...
  useEffect(() => {
    ...
    eventSource.addEventListener('sberbank_changed', refresh);
  ...
  const handlePointTap = async (pointId: number) => {
    ...
    try {
      await axios.post(
        '/api/trpc/sberbank.togglePickup',

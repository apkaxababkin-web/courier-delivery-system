...
export function CreateTaskModal({ isOpen, onClose, onSubmit, clients, isLoading }: CreateTaskModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="max-h-[92vh] w-full max-w-5xl overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-2xl shadow-slate-950/20" overlayStyle={{ background: 'rgba(15,23,42,0.45)', backdropFilter: 'blur(8px)' }}>
      <form onSubmit={handleSubmit} className="flex max-h-[92vh] flex-col">
        <div className="grid gap-4 border-b border-slate-200 px-6 py-4 md:grid-cols-[1fr_340px_auto] md:items-end">
          <div className="self-center">
            <h2 className="text-xl font-semibold tracking-tight text-slate-950">Создать заявку</h2>
            <p className="mt-1 text-sm text-slate-500">Операционная форма без лишних шагов.</p>
          </div>
          {requestTypeSelector}
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-6 py-4">
              ...
              <div className="space-y-4">
                ...
                <Section title="Тарифы">
                  <div className="grid gap-3">
                    ...
                    <p className="text-xs leading-5 text-slate-500">Тарифы отделены от ежедневного ввода коробок и сохраняются между открытиями формы на этом рабочем месте.</p>
                  </div>
                </Section>
              </div>
            </div>
          ...
          {requestType === 'pickup_from_tc' && <><Section title="Клиент"><ClientSelect label="Выберите клиента" value={formData.clientId} clients={clients} onChange={(id) => selectClient(id, 'pickupClient')} /></Section><div className="grid gap-4 lg:grid-cols-2"><Section title="Транспортная компания"><div className="grid gap-3 md:grid-cols-2"><SelectField label="Направление" value={formData.pickupDirection || 'tc_to_recipient'} onChange={(value) => updateField('pickupDirection', value as LocalFormData['pickupDirection'])} options={[["tc_to_recipient", "ТК → получатель"], ["recipient_to_tc", "Получатель → ТК"]]} className="md:col-span-2" /><Field label="Название ТК" value={formData.tcName || ''} onChange={(value) => updateField('tcName', value)} className="md:col-span-2" /><Field label="Адрес ТК" value={formData.tcAddress || ''} onChange={(value) => updateField('tcAddress', value)} className="md:col-span-2" /><Field label="Номер трекинга" value={formData.trackingNumber || ''} onChange={(value) => updateField('trackingNumber', value)} className="md:col-span-2" /></div></Section><Section title="Получатель"><div className="grid gap-3 md:grid-cols-2"><ClientSelect label="Выберите получателя" value={formData.pickupRecipientClientId} clients={clients} onChange={(id) => selectClient(id, 'pickupRecipient')} className="md:col-span-2" /><Field label="Получатель" value={formData.recipientName || ''} onChange={(value) => updateField('recipientName', value)} /><Field label="Телефон получателя" value={formData.recipientPhone || ''} onChange={(value) => updateField('recipientPhone', value)} /><Field label="Адрес доставки" value={formData.deliveryAddress || ''} onChange={(value) => updateField('deliveryAddress', value)} className="md:col-span-2" /><TextareaField label="Комментарии" value={formData.comments || ''} onChange={(value) => updateField('comments', value)} className="md:col-span-2" /></div></Section></div></>}
        </div>

        <div className="sticky bottom-0 flex flex-col-reverse gap-3 border-t border-slate-200 bg-white/95 px-6 py-4 backdrop-blur sm:flex-row sm:justify-end"><button type="button" onClick={onClose} className="inline-flex h-11 items-center justify-center rounded-2xl border border-slate-200 bg-white px-5 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50">Отмена</button><button type="submit" disabled={isLoading} className="inline-flex h-11 items-center justify-center rounded-2xl bg-slate-950 px-5 text-sm font-medium text-white shadow-lg shadow-slate-950/10 hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-60">{isLoading ? 'Создание...' : 'Создать заявку'}</button></div>
      </form>
    </Modal>
  );
}

function Section({ title, children }: { title: string; children: React.ReactNode }) { return <section className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm"><h3 className="mb-3 text-sm font-semibold text-slate-950">{title}</h3>{children}</section>; }
function Field({ label, value, onChange, required, type = 'text', className = '' }: { label: string; value: string | number; onChange: (value: string) => void; required?: boolean; type?: string; className?: string }) { return <div className={className}><label className="mb-1.5 block text-sm font-medium text-slate-700">{label}</label><input type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={label} required={required} className="h-10 w-full rounded-2xl border border-slate-200 bg-slate-50 px-3.5 text-sm outline-none transition focus:border-slate-300 focus:bg-white" /></div>; }
function SelectField({ label, value, onChange, options, className = '' }: { label: string; value: string | number; onChange: (value: string) => void; options: Array<[string | number, string]>; className?: string }) { return <div className={className}><label className="mb-1.5 block text-sm font-medium text-slate-700">{label}</label><select value={value} onChange={(e) => onChange(e.target.value)} className="h-10 w-full rounded-2xl border border-slate-200 bg-slate-50 px-3.5 text-sm outline-none transition focus:border-slate-300 focus:bg-white">{options.map(([optionValue, labelText]) => <option key={String(optionValue)} value={optionValue}>{labelText}</option>)}</select></div>; }
function ClientSelect({ label, value, clients, onChange, className = '' }: { label: string; value?: number; clients: Client[]; onChange: (value: number | undefined) => void; className?: string }) { return <div className={className}><label className="mb-1.5 block text-sm font-medium text-slate-700">{label}</label><select value={value || ''} onChange={(e) => onChange(e.target.value ? Number(e.target.value) : undefined)} className="h-10 w-full rounded-2xl border border-slate-200 bg-slate-50 px-3.5 text-sm outline-none transition focus:border-slate-300 focus:bg-white"><option value="">-- Не выбрано --</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.name} ({client.address})</option>)}</select></div>; }
function TextareaField({ label, value, onChange, className = '' }: { label: string; value: string; onChange: (value: string) => void; className?: string }) { return <div className={className}><label className="mb-1.5 block text-sm font-medium text-slate-700">{label}</label><textarea value={value} onChange={(e) => onChange(e.target.value)} placeholder={label} rows={3} className="w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm outline-none transition focus:border-slate-300 focus:bg-white" /></div>; }

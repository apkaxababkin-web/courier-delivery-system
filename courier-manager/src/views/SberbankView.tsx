...
type SberbankPointRow = api.SberbankPoint & { listId?: number; isArchived?: boolean };
...
export default function SberbankView({ archiveDate }: { archiveDate?: string }) {
  ...
  const [confirmState, setConfirmState] = useState<{ kind: 'removeFromList' | 'archive'; point: SberbankPointRow } | null>(null);
  ...
  const loadPoints = async () => {
    try {
      ...
      const rows: SberbankPointRow[] = data.map((point) => ({
        ...point,
  ...
  const openRemoveFromList = (point: SberbankPointRow) => {
    setFeedback(null);
    setConfirmError(null);
    setConfirmState({ kind: 'removeFromList', point });
  };

  const openArchivePoint = (point: SberbankPointRow) => {
    setFeedback(null);
    setConfirmError(null);
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (kind === 'removeFromList') {
        ...
        await api.deleteSberbankPoint(point.id);
        setSelectedPoints((prev) => prev.filter((id) => id !== point.id));
                ...
                {visiblePoints.map((point) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      className="h-4 w-4 cursor-pointer rounded border-slate-300 text-slate-950 focus:ring-2 focus:ring-slate-300 disabled:cursor-not-allowed disabled:opacity-40"
                    />

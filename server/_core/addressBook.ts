...
async function audit(tx: any, managerId: number | null, action: string, entity: string, entityId: number, before: unknown, after: unknown) {
  // "id" has no sequence default in this database and occurredAt has no NOT NULL
  // default guarantee everywhere, so both are supplied explicitly. The advisory
  // lock serialises concurrent inserts in this module. The table schema itself is
  // not modified.
  await tx.execute(sql`SELECT pg_advisory_xact_lock(738492, 1)`);
  await tx.execute(sql`INSERT INTO "correspondenceAuditLog" ("id","managerId","occurredAt","action","entityType","entityId","beforeData","afterData")
    SELECT COALESCE(max("id"), 0) + 1, ${managerId}, now(), ${action}, ${entity}, ${entityId},
      ${before ? JSON.stringify(before) : null}::jsonb, ${after ? JSON.stringify(after) : null}::jsonb
      FROM "correspondenceAuditLog"`);
}

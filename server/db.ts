...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  try {
    ...
    await db.insert(users).values(values).onConflict((t: any) => ({
      target: t.openId,
...
export async function getTasksByDateWithCourier(dateStr: string): Promise<TaskWithCourier[]> {
  const db = await getDb();
  if (!db) return [];

  // Local business day for Ulan-Ude / Asia/Irkutsk (UTC+8).
  // Example: 2026-06-17 local day = 2026-06-16 16:00 UTC ... 2026-06-17 15:59 UTC.
